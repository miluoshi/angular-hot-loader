...
module.exports = function (name, directive) {
  ...
  this.templateCache[name] = def.template && def.template.toString();
...
function hotUpdateDirective(name, newDirective) {
  ...
  const {
    controller, controllerAs, link, require, scope, template, templateUrl, transclude, bindToController, restrict
  } = newDirective
